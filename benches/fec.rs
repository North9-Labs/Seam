...
fn bench_fec_encode(c: &mut Criterion) {
    ...
    for (k, r, size) in [(4u8, 2u8, 1400usize), (8, 2, 1400), (10, 3, 1400)] {
        ...
        group.bench_with_input(BenchmarkId::new("encode", &label), &(k, r, size), |b, &(k, r, size)| {
...
fn bench_fec_decode_no_loss(c: &mut Criterion) {
    ...
    let k = 8u8; let r = 2u8; let size = 1400usize;
    let sources = make_sources(k, size);
    ...
    group.bench_function("k8r2_1400B", |b| {
        let mut enc = FecEncoder::new(0, k, r);
        for src in &sources { enc.push_source(src); }
        b.iter(|| {
...
fn bench_fec_recover_1_loss(c: &mut Criterion) {
    ...
    for (k, r, size) in [(4u8, 1u8, 1400usize), (8, 2, 1400), (10, 2, 1400)] {
        ...
        group.bench_with_input(BenchmarkId::new("recover", &label), &(k, r, size), |b, &(k, r, size)| {
            let sources = make_sources(k, size);
            let mut enc = FecEncoder::new(0, k, r);
            for src in &sources { enc.push_source(src); }
            ...
            let repairs = {
                let mut rs = None;
                for src in &sources { rs = enc2.push_source(src); }
                rs.unwrap()
            };
...
criterion_group!(benches, bench_gf_mul_slice, bench_fec_encode, bench_fec_decode_no_loss, bench_fec_recover_1_loss);
